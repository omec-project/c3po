#ifndef KMS_CLIENT_H_
#define KMS_CLIENT_H_
...
#define DFL_ETM                 -1
#define DFL_KEY_NAMESPACE	"dealer_default"
#define KEY_NAMESPACE_LEN	33
...
typedef struct options {
	...
	char key_namespace[KEY_NAMESPACE_LEN];
} client_opt_t;

__attribute__((unused)) static void client_opt_init(client_opt_t* opt) {
	...
	memset(opt->key_namespace, 0, KEY_NAMESPACE_LEN);
}
...
int kms_client(client_opt_t opt, char* headers[], int n_header,
		unsigned char* output, int length, mbedtls_pk_context*,
		mbedtls_x509_crt*, mbedtls_x509_crt*);

extern char kms_mrenclave[SGX_HASH_SIZE * 2 + 1];
