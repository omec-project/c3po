...
bool Options::parseJson()
{
   ...
   if(doc.HasMember("tssf")){
      const RAPIDJSON_NAMESPACE::Value& tssfSection = doc["tssf"];
      if(!(m_options & opt_pcrfhost) && tssfSection.HasMember("pcrfhost")){
         if(!tssfSection["pcrfhost"].IsString()) { std::cout << "Error parsing json value: [pcrfhost]" << std::endl; return false; }
         m_pcrfhost = tssfSection["pcrfhost"].GetString();
         m_options |= opt_pcrfhost;
      }
      if(!(m_options & opt_pcrfrealm) && tssfSection.HasMember("pcrfrealm")){
         if(!tssfSection["pcrfrealm"].IsString()) { std::cout << "Error parsing json value: [pcrfrealm]" << std::endl; return false; }
         m_pcrfrealm = tssfSection["pcrfrealm"].GetString();
         m_options |= opt_pcrfrealm;
      }
      if(!(m_options & opt_casshost) && tssfSection.HasMember("casshost")){
         if(!tssfSection["casshost"].IsString()) { std::cout << "Error parsing json value: [casshost]" << std::endl; return false; }
         m_cassHost = tssfSection["casshost"].GetString();
         m_options |= opt_casshost;
      }
      if(!(m_options & opt_cassks) && tssfSection.HasMember("casskeyspace")){
         if(!tssfSection["casskeyspace"].IsString()) { std::cout << "Error parsing json value: [casskeyspace]" << std::endl; return false; }
         m_cassKeyspace = tssfSection["casskeyspace"].GetString();
         m_options |= opt_cassks;
      }
      if(!(m_options & opt_logmaxsize) && tssfSection.HasMember("logsize")){
         if(!tssfSection["logsize"].IsInt()) { std::cout << "Error parsing json value: [logsize]" << std::endl; return false; }
         m_logmaxsize = tssfSection["logsize"].GetInt();
         m_options |= opt_logmaxsize;
      }
      if(!(m_options & opt_lognbrfiles) && tssfSection.HasMember("lognumber")){
         if(!tssfSection["lognumber"].IsInt()) { std::cout << "Error parsing json value: [lognumber]" << std::endl; return false; }
         m_lognbrfiles = tssfSection["lognumber"].GetInt();
         m_options |= opt_lognbrfiles;
      }
      if(!(m_options & opt_logfilename) && tssfSection.HasMember("logname")){
         if(!tssfSection["logname"].IsString()) { std::cout << "Error parsing json value: [logname]" << std::endl; return false; }
         m_logfilename = tssfSection["logname"].GetString();
         m_options |= opt_logfilename;
      }
      if(!(m_options & opt_logqueuesize) && tssfSection.HasMember("logqsize")){
         if(!tssfSection["logqsize"].IsInt()) { std::cout << "Error parsing json value: [logqsize]" << std::endl; return false; }
         m_logqueuesize = tssfSection["logqsize"].GetInt();
