...
CassandraDataAccess::CassandraDataAccess()
{
}
...
void CassandraDataAccess::connect( const std::string &hst, const std::string &ks )
{
   std::cout << "Connecting to Cassandra DB" << std::endl;
   host( hst );
   keyspace( ks );
   connect();
}

void CassandraDataAccess::connect( const char *hst, const char *ks )
{
   std::cout << "Connecting to Cassandra DB" << std::endl;
   host( hst );
   keyspace( ks );
...
void CassandraDataAccess::connect()
{
   ...
   {
      throw DAException(
      SUtility::string_format( "CassandraDataAccess::%s - Unable to connect to %s - error_code=%d",
...
void CassandraDataAccess::disconnect()
{
    m_db.disconnect();
}
...
bool CassandraDataAccess::addEvent( DAEvent &event )
{
    std::stringstream ss;
    // insert the event
    {
    ss << "INSERT INTO events ("
       ...
       {
          throw DAException(
             SUtility::string_format( "CassandraDataAccess::%s - Error %d executing [%s]",
...
bool CassandraDataAccess::getEvent( const char *scef_id, uint32_t scef_ref_id, DAEvent &event )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getEvents( const char *scef_id, std::list<uint32_t> scef_ref_ids, DAEventList &events, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
   ...
   if (cb)
      return future.setCallback((CassFutureCallback)cb, data);

   return getEventsData( future, events );
}

bool CassandraDataAccess::getEventsData( SCassFuture &future, DAEventList &events )
{
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error( "CassandraDataAccess::%s - Error %d executing getEvents()",
...
void CassandraDataAccess::deleteEvent( const char *scef_id, uint32_t scef_ref_id )
{
   DAEvent event;
   std::stringstream ss;
...
bool CassandraDataAccess::checkMSISDNExists( int64_t msisdn )
{
   std::stringstream ss;
...
bool CassandraDataAccess::checkImsiExists( const char *imsi )
{
   std::stringstream ss;
...
bool CassandraDataAccess::checkExtIdExists( const char *extid )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getImsiListFromExtId( const char *extid, DAImsiList &imsilst)
{
   std::stringstream ss;
...
bool CassandraDataAccess::getExtIdsFromImsiData( SCassFuture &future, DAExtIdList &extids )
{
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error( "CassandraDataAccess::%s - Error %d executing getExtIdsFromImsi()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::getExtIdsFromImsi( const char *imsi, DAExtIdList &extids, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getImsiFromMsisdn( int64_t msisdn, std::string &imsi )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getImsiFromMsisdn( const char *msisdn, std::string &imsi )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getMsisdnFromImsi( const char *imsi, std::string &msisdn )
{
    std::stringstream ss;
...
bool CassandraDataAccess::getMsisdnFromImsi( const char *imsi, int64_t &msisdn )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getImsiInfoData( SCassFuture &future, DAImsiInfo &info )
{
   if ( future.errorCode() != CASS_OK ) {
      throw DAException(
            SUtility::string_format( "CassandraDataAccess::%s - Error %d executing getImsiInfo()",
                  __func__, future.errorCode() )
      );
   }
...
bool CassandraDataAccess::getImsiInfo( const char *imsi, DAImsiInfo &info, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
...
bool CassandraDataAccess::insertUserImsi( const ImsiEntity &ie, DataAccessCallback cb, void *data)
{
   std::stringstream ss;
...
bool CassandraDataAccess::getEventIdsFromMsisdnData( SCassFuture &future, DAEventIdList &eil )
{
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error( "CassandraDataAccess::%s - Error %d executing getEventIdsFromMsisdn()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::getEventIdsFromMsisdn( int64_t msisdn, DAEventIdList &eil, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
   ...
   if (cb)
   {
      bool rval = future.setCallback( (CassFutureCallback)cb, data );
      if (!rval)
         printf("%s:%d - CassandraDataAccess::getEventIdsFromMsisdn() failed CassError (%d)\n", __FILE__, __LINE__, future.errorCode());
      return rval;
   }
...
void CassandraDataAccess::getEventIdsFromExtId( const char *extid, DAEventIdList &eil )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getEventIdsFromExtIds( const char *extids, DAEventIdList &el, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getEventIdsFromExtIdsData( SCassFuture &future, DAEventIdList &el)
{
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error( "CassandraDataAccess::%s - Error %d executing getEventIdsFromExtIds()",
            __func__, future.errorCode() );
      return false;
   }
   ...
   {
      Logger::system().error( "CassandraDataAccess::%s - EXCEPTION - %s",
            __func__, ex.what() );
      return false;
   }
...
void CassandraDataAccess::getEventsFromImsi( const char *imsi, DAEventList &el )
{
   DAImsiInfo info;
...
void CassandraDataAccess::getEventsFromImsi( DAImsiInfo &info, DAEventList &el )
{
...
bool CassandraDataAccess::checkOpcKeys( const uint8_t opP[16] )
{
...
bool CassandraDataAccess::updateOpc ( std::string &imsi, std::string& opc )
{
   std::stringstream ss;
...
bool CassandraDataAccess::purgeUE ( std::string &imsi )
{
   if (imsi.empty())
      return false;
...
bool CassandraDataAccess::getMmeIdentityFromImsi ( std::string &imsi, DAMmeIdentity& mmeid)
{
   std::stringstream ss;
...
bool CassandraDataAccess::getMmeIdentity ( std::string &mme_id, DAMmeIdentity& mmeid )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getMmeIdentity ( int32_t mme_id, DAMmeIdentity& mmeid )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getLatestIdentity(const char *table_name, int64_t &id, DataAccessCallback cb, void *data)
{
   std::stringstream ss;
...
bool CassandraDataAccess::getLatestIdentityData(SCassFuture &future, int64_t &id)
{
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error( "CassandraDataAccess::%s - Error %d executing getLatestIdentity()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::updateLatestIdentity(const char *table_name, DataAccessCallback cb, void *data)
{
   std::stringstream ss;
   ...
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error("CassandraDataAccess::%s - Error %d executing updateLatestIdentity()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::getMmeIdFromHostData( SCassFuture &future, int32_t &mmeid )
{
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error("CassandraDataAccess::%s - Error %d executing getMmeIdFromHost()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::getMmeIdFromHost ( std::string& host, int32_t &mmeid, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
...
bool CassandraDataAccess::addMmeIdentity(std::string &host, std::string &realm, int32_t mmeid)
{
...
bool CassandraDataAccess::addMmeIdentity1(std::string &host, std::string &realm, int32_t mmeid, DataAccessCallback cb, void *data)
{
   std::stringstream ss;
   ...
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error("CassandraDataAccess::%s - Error %d executing addMmeIdentity1()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::addMmeIdentity2(std::string &host, std::string &realm, int32_t mmeid, DataAccessCallback cb, void *data)
{
   std::stringstream ss;
   ...
   if ( future.errorCode() != CASS_OK )
   {
      Logger::system().error("CassandraDataAccess::%s - Error %d executing addMmeIdentity2()",
            __func__, future.errorCode() );
      return false;
   }
...
bool CassandraDataAccess::updateLocation( DAImsiInfo &location, uint32_t present_flags, int32_t idmmeidentity, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
...
bool CassandraDataAccess::updateLocation ( DAImsiInfo &location, uint32_t present_flags, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
   ss << "UPDATE vhss.users_imsi SET ";
...
bool CassandraDataAccess::getImsiSecData(SCassFuture &future, DAImsiSec &imsisec)
{
   if ( future.errorCode() != CASS_OK )
   {
       throw DAException(
           SUtility::string_format( "CassandraDataAccess::%s - Error %d executing getImsiSec()",
           __func__, future.errorCode() )
       );
   }
...
bool CassandraDataAccess::getImsiSec ( const std::string &imsi, DAImsiSec &imsisec, DataAccessCallback cb, void *data )
{
   std::stringstream ss;
...
bool CassandraDataAccess::updateRandSqn ( const std::string &imsi, uint8_t * rand_p, uint8_t * sqn, bool inc_sqn, DataAccessCallback cb, void *data )
{
   SqnU64Union eu;
...
bool CassandraDataAccess::incSqn ( std::string &imsi, uint8_t * sqn )
{
   SqnU64Union eu;
...
bool CassandraDataAccess::getSubDataFromImsi( const char *imsi, std::string &sub_data )
{
    std::stringstream ss;
...
void CassandraDataAccess::UpdateValidityTime( const char *imsi, std::string &validity_time )
{
   std::stringstream ss;
...
void CassandraDataAccess::UpdateNIRDestination( const char *imsi, std::string &host, std::string &realm )
{
   std::stringstream ss;
...
bool CassandraDataAccess::getImsiFromScefIdScefRefId( char *scef_id, uint32_t scef_ref_id, std::string &imsi )
{
    std::stringstream ss;
...
bool CassandraDataAccess::getScefIdScefRefIdFromImsi( char *imsi, std::string &scef_id, uint32_t &scef_ref_id )
{
    std::stringstream ss;
...
bool CassandraDataAccess::deleteEvent( std::string scef_id, uint32_t scef_ref_id )
{
    std::stringstream ss;
...
bool CassandraDataAccess::delScefIds( const char *imsi )
{
    std::stringstream ss;
...
bool CassandraDataAccess::delImsiFromScefIds ( const char *scef_id, uint32_t scef_ref_id )
{
    std::stringstream ss;
...
bool CassandraDataAccess::CheckImsiAttached ( char *imsi )
{
    std::stringstream ss;
...
void CassandraDataAccess::getMonEventConfForImsi( const char *imsi, DAMonitoringConfEventList &mcel )
{
   std::stringstream ss;
